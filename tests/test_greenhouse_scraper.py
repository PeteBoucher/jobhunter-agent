"""Tests for Greenhouse job board scraper."""

import tempfile
from datetime import datetime
from unittest.mock import MagicMock, patch

import pytest

from src.database import get_session, init_db
from src.job_scrapers.greenhouse_scraper import GreenhouseScraper
from src.models import Job


@pytest.fixture
def temp_db():
    """Create temporary database for testing."""
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = f"sqlite:///{tmpdir}/test.db"
        import os

        os.environ["DATABASE_URL"] = db_path
        init_db()
        yield db_path


@pytest.fixture
def session(temp_db):
    """Get database session."""
    session = get_session()
    yield session
    session.close()


@pytest.fixture
def scraper(session):
    """Create Greenhouse scraper with a small set of boards."""
    return GreenhouseScraper(session, board_tokens=["testcompany"])


# Sample Greenhouse API response
SAMPLE_GREENHOUSE_RESPONSE = {
    "jobs": [
        {
            "id": 12345,
            "title": "Senior Software Engineer",
            "updated_at": "2025-12-01T10:00:00Z",
            "absolute_url": "https://boards.greenhouse.io/testcompany/jobs/12345",
            "location": {"name": "San Francisco, CA"},
            "departments": [{"id": 1, "name": "Engineering"}],
            "content": (
                "<p>We are looking for a Senior Software Engineer.</p>"
                "<h3>Requirements</h3>"
                "<ul><li>5+ years of Python experience</li>"
                "<li>Experience with Docker and Kubernetes</li></ul>"
            ),
        },
        {
            "id": 67890,
            "title": "Product Manager - Remote",
            "updated_at": "2025-11-15T08:30:00Z",
            "absolute_url": "https://boards.greenhouse.io/testcompany/jobs/67890",
            "location": {"name": "Remote - US"},
            "departments": [{"id": 2, "name": "Product"}],
            "content": "<p>Join our product team to shape the future.</p>",
        },
    ]
}


class TestGreenhouseScraper:
    """Tests for Greenhouse scraper."""

    def test_source_name(self, scraper):
        """Test that Greenhouse scraper has correct source name."""
        assert scraper._get_source_name() == "greenhouse"

    def test_parse_job_basic(self, scraper):
        """Test parsing a standard Greenhouse job."""
        raw_job = SAMPLE_GREENHOUSE_RESPONSE["jobs"][0].copy()
        raw_job["_board_token"] = "testcompany"

        parsed = scraper._parse_job(raw_job)

        assert parsed["source_job_id"] == "12345"
        assert parsed["title"] == "Senior Software Engineer"
        assert parsed["company"] == "Testcompany"
        assert parsed["location"] == "San Francisco, CA"
        assert parsed["department"] == "Engineering"
        assert parsed["remote"] is None
        assert parsed["source_type"] == "company_portal"
        assert (
            parsed["apply_url"] == "https://boards.greenhouse.io/testcompany/jobs/12345"
        )
        assert "python" in parsed["requirements"]
        assert "docker" in parsed["requirements"]
        assert "kubernetes" in parsed["requirements"]

    def test_parse_job_remote(self, scraper):
        """Test parsing a remote job correctly detects remote status."""
        raw_job = SAMPLE_GREENHOUSE_RESPONSE["jobs"][1].copy()
        raw_job["_board_token"] = "testcompany"

        parsed = scraper._parse_job(raw_job)

        assert parsed["title"] == "Product Manager - Remote"
        assert parsed["remote"] == "remote"
        assert parsed["location"] == "Remote - US"

    def test_parse_job_posted_date(self, scraper):
        """Test that posted date is parsed correctly."""
        raw_job = SAMPLE_GREENHOUSE_RESPONSE["jobs"][0].copy()
        raw_job["_board_token"] = "testcompany"

        parsed = scraper._parse_job(raw_job)

        assert isinstance(parsed["posted_date"], datetime)
        assert parsed["posted_date"].year == 2025
        assert parsed["posted_date"].month == 12

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_fetch_jobs_success(self, mock_get, scraper):
        """Test successful job fetching from Greenhouse API."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = SAMPLE_GREENHOUSE_RESPONSE
        mock_get.return_value = mock_response

        jobs = scraper._fetch_jobs()

        assert len(jobs) == 2
        assert jobs[0]["_board_token"] == "testcompany"
        assert jobs[0]["title"] == "Senior Software Engineer"

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_fetch_jobs_404(self, mock_get, scraper):
        """Test handling of 404 (board not found)."""
        mock_response = MagicMock()
        mock_response.status_code = 404
        mock_get.return_value = mock_response

        jobs = scraper._fetch_jobs()

        assert len(jobs) == 0

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_fetch_jobs_network_error(self, mock_get, scraper):
        """Test handling of network errors."""
        import requests

        mock_get.side_effect = requests.RequestException("Connection timeout")

        jobs = scraper._fetch_jobs()

        assert len(jobs) == 0

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_scrape_persists_jobs(self, mock_get, scraper):
        """Test that scrape() persists jobs to database."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = SAMPLE_GREENHOUSE_RESPONSE
        mock_get.return_value = mock_response

        jobs = scraper.scrape(max_retries=1)

        assert len(jobs) == 2

        # Verify jobs are in database
        saved = scraper.session.query(Job).filter(Job.source == "greenhouse").all()
        assert len(saved) == 2

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_scrape_deduplicates(self, mock_get, scraper):
        """Test that scraping twice doesn't create duplicates."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = SAMPLE_GREENHOUSE_RESPONSE
        mock_get.return_value = mock_response

        first = scraper.scrape(max_retries=1)
        second = scraper.scrape(max_retries=1)

        assert len(first) == 2
        assert len(second) == 0  # All duplicates

        saved = scraper.session.query(Job).filter(Job.source == "greenhouse").all()
        assert len(saved) == 2

    @patch("src.job_scrapers.greenhouse_scraper.requests.get")
    def test_scrape_by_keywords(self, mock_get, scraper):
        """Test keyword filtering."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = SAMPLE_GREENHOUSE_RESPONSE
        mock_get.return_value = mock_response

        count = scraper.scrape_by_keywords(["software engineer"])

        assert count == 1  # Only the "Senior Software Engineer" matches

    def test_extract_requirements(self, scraper):
        """Test requirement extraction from description text."""
        description = (
            "Experience with Python, Docker, and AWS required. Kubernetes a plus."
        )
        reqs = scraper._extract_requirements(description)

        assert "python" in reqs
        assert "docker" in reqs
        assert "aws" in reqs
        assert "kubernetes" in reqs

    def test_extract_requirements_empty(self, scraper):
        """Test requirement extraction with no matches."""
        reqs = scraper._extract_requirements("No technical skills mentioned here.")
        assert reqs is None

    def test_extract_requirements_none_input(self, scraper):
        """Test requirement extraction with None input."""
        reqs = scraper._extract_requirements("")
        assert reqs is None
