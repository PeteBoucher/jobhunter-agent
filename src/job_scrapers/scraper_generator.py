...
def generate_scraper(url: str, output_path: Optional[str] = None) -> str:
    ...
    print(
        ...
        f"({len(api_samples)} live endpoint(s) found)"
    )
    ...
    out_path.write_text(
        ...
        f"# Review, test with pytest, then add to src/job_scrapers/registry.py\n\n"
        + code
    )
